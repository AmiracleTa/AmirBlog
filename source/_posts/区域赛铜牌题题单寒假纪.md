...
注意到了一定是先除2，若不考虑除2操作，则是仓库选址的问题。若已经进行了若干次除2操作，最终的目标数是确定的，是中位数。注意到这个可能的中位数只有 nlog 个，我们可以枚举这个可能的中位数作为目标数，然后枚举计算每个元素变成目标书的最小花费，去掉 m 个较大的即可
